...
const WorkspaceSidebar: React.FC = () => {
    ...
    const handleBoardClick = (boardId: number) => {
        ...
        const targetPath = `/boards/${boardId}`;
                        ...
                        {boards.map((board) => (
                            <li key={board.id} className="group flex items-center justify-between rounded-lg hover:bg-gray-700">
                                <button
                                    ...
                                    className={`flex-grow flex items-center p-2 rounded-lg transition-all duration-200 text-sm font-medium text-left ${selectedBoardId === board.id ? 'bg-main-purple text-white' : ''
                                        }`}
                                >
